...
import { moneyToCentimes, normalizeMoneyInput } from '../utils/money';

const toCentimes = moneyToCentimes;
...
export function BudgetEditor({ data, month, onClose, onSaved }) {
  ...
  const submit = async event => {
    ...
    try { await api.updateBudget({ ...form, dailyBudget: normalizeMoneyInput(form.dailyBudget).trim(), monthlyBudget: normalizeMoneyInput(form.monthlyBudget).trim(), periodDays }); await onSaved(); onClose(); }
  ...
  return <div className="modal-layer"><button className="scrim" aria-label="Close" onClick={onClose}/><section className="settings-modal" role="dialog" aria-modal="true" aria-labelledby="budget-title"><header><div><span className="eyebrow">Flexible spending period</span><h2 id="budget-title">{title}</h2></div><button className="icon-btn" onClick={onClose} aria-label="Close budget settings"><X/></button></header><p>Choose when this budget starts and how many days it should cover. We suggest a safe daily target, and you can lower it whenever you prefer.</p><form className="auth-form" onSubmit={submit}>
    <label>Period budget (DH)<input type="text" aria-describedby="budget-suggestion" inputMode="decimal" pattern="[0-9]+([.][0-9]{1,2})?" value={form.monthlyBudget} onChange={event => setForm(applySuggestion({ monthlyBudget: normalizeMoneyInput(event.target.value) }))}/></label>
    ...
    <label>Daily target (DH)<input type="text" inputMode="decimal" pattern="[0-9]+([.][0-9]{1,2})?" value={form.dailyBudget} onChange={event => { setDailyTouched(true); setForm({ ...form, dailyBudget: normalizeMoneyInput(event.target.value) }); }}/></label>
