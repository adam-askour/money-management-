...
export function Ledger({ days, onOpen }) {
  return <section className="ledger" aria-labelledby="ledger-title">
    <div className="section-heading"><div><span className="eyebrow">01 — 31 August 2026</span><h2 id="ledger-title">The monthly<br/>money trail.</h2></div><p>Every day has a place. Open a row to review details or add an expense when the edit window is active.</p></div>
    <div className="ledger-list">
      {days.map((day, index) => <motion.button type="button" className="day-row" key={day.date} onClick={() => onOpen(day)} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: Math.min(index * .018, .25) }} aria-label={`View ${day.fullDate} expense details`}>
        ...
        <span className="day-difference">{day.recorded ? formatDH(day.differenceCentimes, true) : '—'}</span>
