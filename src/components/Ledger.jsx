...
export function Ledger({ days, monthName, onOpen }) {
  return <section className="ledger" aria-labelledby="ledger-title">
    <div className="section-heading"><div><span className="eyebrow">{days[0]?.displayDate} — {days.at(-1)?.displayDate}</span><h2 id="ledger-title">The monthly<br/>money trail.</h2></div><p>{monthName}. Every day has a place. Open a row to review details or add an expense when the edit window is active.</p></div>
