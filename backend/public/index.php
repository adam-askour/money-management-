...
try{
 ...
 if($method==='GET'&&$path==='/admin/users'){$auth->requireAdmin();JsonResponse::success(['users'=>$authRepo->users(),'invitations'=>$authRepo->invitations(),'limit'=>50,'used'=>$authRepo->countUsers()]);}
 ...
 if($method==='DELETE'&&preg_match('#^/admin/invitations/(\d+)$#',$path,$m)){$auth->requireAdmin();if(!$authRepo->revokeInvitation((int)$m[1]))JsonResponse::error('Invitation not found or no longer pending.',404);JsonResponse::success(['revoked'=>true]);}
