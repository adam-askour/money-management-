...
final class MoneyService {
 ...
 public function month(string $month):array {
  ...
  foreach(new DatePeriod($start,new DateInterval('P1D'),$end->modify('+1 day')) as $date){
   ...
   $difference=$budget===null?null:$budget-$total;$edit=$this->dates->state($key);
   $closed=$edit['state']==='locked';
   $counted=$recorded||(!$recorded&&$closed&&$difference!==null);$state=$this->budgetState($counted,$difference);
   if($recorded){$recordedCount++;$totalSpent+=$total;}
   if($counted&&$difference!==null){if($difference>0){$saved+=$difference;$underCount++;}elseif($difference<0){$over+=abs($difference);$overCount++;}else$onCount++;}
   elseif(!$recorded&&$edit['state']!=='before_activation')$emptyCount++;
   $day=['date'=>$key,'fullDate'=>$date->format('l, j F Y'),'weekday'=>$date->format('l'),'displayDate'=>$date->format('j F Y'),'recorded'=>$recorded,'expenseCount'=>count($expenses),'preview'=>array_slice(array_column($expenses,'description'),0,2),'totalCentimes'=>$total,'dailyBudgetCentimes'=>$budget,'differenceCentimes'=>$counted?$difference:null,'budgetState'=>$state['key'],'budgetMessage'=>$state['label'],'editable'=>$edit['editable'],'editState'=>$edit['state'],'editLabel'=>$edit['label']];
 ...
 public function day(string $date):array{$edit=$this->dates->state($date);$expenses=array_map(fn($e)=>['id'=>(int)$e['id'],'description'=>$e['description'],'amountCentimes'=>(int)$e['amount_centimes']],$this->repo->dayExpenses($this->userId,$date));$recorded=(bool)$expenses;$total=array_sum(array_column($expenses,'amountCentimes'));$budget=$this->repo->budget($this->userId,$date)['daily'];$difference=$budget===null?null:$budget-$total;$closed=$edit['state']==='locked';$counted=$recorded||(!$recorded&&$closed&&$difference!==null);$state=$this->budgetState($counted,$difference);return['date'=>$date,'expenses'=>$expenses,'recorded'=>$recorded,'totalCentimes'=>$total,'dailyBudgetCentimes'=>$budget,'differenceCentimes'=>$counted?$difference:null,'budgetState'=>$state['key'],'budgetMessage'=>$state['label'],'editable'=>$edit['editable'],'editState'=>$edit['state'],'editLabel'=>$edit['label']];}
 ...
 private function budgetState(bool $counted,?int $difference):array{if($difference===null)return['key'=>'empty','label'=>'Set your budget'];if(!$counted)return['key'=>'empty','label'=>'No expenses recorded'];if($difference>0)return['key'=>'under','label'=>'Saved '.$this->dh($difference)];if($difference<0)return['key'=>'over','label'=>'Over budget by '.$this->dh(abs($difference))];return['key'=>'on','label'=>'On budget'];}
