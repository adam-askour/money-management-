...
final class AuthService {
 ...
 public function invite(array $data,int $adminId):array{if($this->repo->countUsers() >= $this->maxUsers)throw new \DomainException('The 50-user account limit has been reached.');if($this->repo->findByEmail($data['email']))throw new \InvalidArgumentException('An account already exists for this email.');$token=bin2hex(random_bytes(32));$expires=(new \DateTimeImmutable('+24 hours'))->format('Y-m-d H:i:s');$this->repo->createInvitation($data['name'],$data['email'],hash('sha256',$token),$adminId,$expires);return['token'=>$token,'expiresAt'=>$expires];}
 ...
 private function publicUser(array $u):array{return['id'=>(int)$u['id'],'name'=>$u['name'],'email'=>$u['email'],'role'=>$u['role'],'isAdmin'=>$u['role']==='admin','activationDate'=>$u['activation_date']??substr((string)($u['activated_at']??$u['created_at']??''),0,10)];}
}
